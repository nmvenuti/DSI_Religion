...
filepath='C:/Users/nmvenuti/Documents/GitHub/dsicapstone-predicting_extremism/nmvenuti_sandbox/spring break updates/ref_complete1-oversample'
...
#Oversample
dat$overSample<-1
dat$overSample[dat$groupRank==4]<-0
n<-ncol(dat)
output<-dat$overSample
input<-dat[,-n]
datOS<-ubUnder(X=input,Y=output, perc = 50, method = "percPos", w = NULL)
nrow(datOS$X)
length(datOS$Y)
datOS<-cbind(datOS$X,datOS$Y)

#Split into test and training
testDat<-dat[grep("test", dat$group),]
trainDat<-dat[grep("train", dat$group),]

testDatOS<-datOS[grep("test", datOS$group),]
trainDatOS<-datOS[grep("train", datOS$group),]
...
model1<-testVariables(x,y,testDatOS,trainDatOS,hiddenLayers,thresholdValue,learningrate,activationFunction,trees, MSE = F)
...
model2<-testVariables(x,y,testDatOS,trainDatOS,hiddenLayers,thresholdValue,learningrate,activationFunction,trees, MSE = F)
...
model3<-testVariables(x,y,testDatOS,trainDatOS,hiddenLayers,thresholdValue,learningrate,activationFunction,trees, MSE = F)
...
model4<-testVariables(x,y,testDatOS,trainDatOS,hiddenLayers,thresholdValue,learningrate,activationFunction,trees, MSE = F)
...
model5<-testVariables(x,y,testDatOS,trainDatOS,hiddenLayers,thresholdValue,learningrate,activationFunction,trees, MSE = F)
...
model6<-testVariables(x,y,testDatOS,trainDatOS,hiddenLayers,thresholdValue,learningrate,activationFunction,trees, MSE = F)
...
model7<-testVariables(x,y,testDatOS,trainDatOS,hiddenLayers,thresholdValue,learningrate,activationFunction,trees, MSE = F)
write.csv(model7[1],file = paste0('model_accuracy-',strName,'.csv'))
write.csv(model7[2],file = paste0('model_output-',strName,'.csv'))

strName<-'ACOM'
x<-c('acom')
model8<-testVariables(x,y,testDatOS,trainDatOS,hiddenLayers,thresholdValue,learningrate,activationFunction,trees, MSE = F)
write.csv(model8[1],file = paste0('model_accuracy-',strName,'.csv'))
write.csv(model8[2],file = paste0('model_output-',strName,'.csv'))

strName<-'Semco'
x<-c('averageSemco')
model9<-testVariables(x,y,testDatOS,trainDatOS,hiddenLayers,thresholdValue,learningrate,activationFunction,trees, MSE = F)
...
x<-c('avgPercJ')
model10<-testVariables(x,y,testDatOS,trainDatOS,hiddenLayers,thresholdValue,learningrate,activationFunction,trees, MSE = F)
...
x<-c('avgNumJ')
model11<-testVariables(x,y,testDatOS,trainDatOS,hiddenLayers,thresholdValue,learningrate,activationFunction,trees, MSE = F)
...
model12<-testVariables(x,y,testDatOS,trainDatOS,hiddenLayers,thresholdValue,learningrate,activationFunction,trees, MSE = F)
...
model13<-testVariables(x,y,testDatOS,trainDatOS,hiddenLayers,thresholdValue,learningrate,activationFunction,trees, MSE = F)
...
model14<-testVariables(x,y,testDatOS,trainDatOS,hiddenLayers,thresholdValue,learningrate,activationFunction,trees, MSE = F)
...
library(gbm)
dataFormula<-paste(c(y," ~ ",paste(x, collapse ="+")),collapse = "")
gbmTest<-gbm(groupRank ~ X.PosWords+X.NegWords+acom+averageSemco+avgPercJ+avgNumJ+eigenvector_centrality+subgraph_centrality, data = trainDatOS, n.trees = 10000)
gbmPredictTest<-predict.gbm(gbmTest,testDatOS, n.trees = 10000)
gbmPredictTest
nrow(testDatOS[abs(testDatOS$groupRank-gbmPredictTest)<1,])/nrow(testDatOS)
# 0.8265306122
